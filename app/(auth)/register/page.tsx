  ...
  const handleRegister = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (axios.isAxiosError(err)) {
        ...
        setError(
          typeof apiMessage === 'string' ? apiMessage : 'Falha ao cadastrar.',
        )
      } else {
  ...
  return (
    <section className="relative flex min-h-screen items-center justify-center overflow-hidden bg-[#070B14] px-4 py-12">
      ...
      <div className="w-full max-w-md rounded-[30px] border border-white/10 bg-gradient-to-b from-white/10 to-white/[0.03] p-8 text-white shadow-[0_24px_80px_rgba(0,0,0,0.55)] backdrop-blur-md">
        <h1 className="mt-2 text-4xl font-bold tracking-tight">Crie sua conta</h1>
        <p className="mt-2 text-base text-white/70">Comece sua jornada com a gente</p>

        <form onSubmit={handleRegister} className="mt-10 space-y-5">
          <div className="space-y-2">
            <label htmlFor="name" className="text-sm text-white/85">
              Nome
            </label>
            <input
              id="name"
